...
def branch_contains(repo_root: Path, branch: str, required_ref: str) -> bool:
    checked = git(repo_root, "merge-base", "--is-ancestor", required_ref, branch, check=False)
...
def hydrate_runtime_worktree(source_root: Path, target_root: Path, *, copy_project_docs: bool) -> None:
    ensure_runs_link(source_root, target_root)
    for relative in (Path("harness"), Path("harness_codex"), Path(".codex/agents"), Path(".codex/skills"), Path(".harness/workflows")):
        mirror_path(source_root / relative, target_root / relative, symlink=True)
    if not copy_project_docs:
        return
    for relative in (
        Path("docs/changes"), Path("docs/use-cases"), Path("docs/plans"), Path("docs/design"),
        Path(".codex/repository-settings.md"), Path(".codex/test-gate.yaml"),
        Path("AGENTS.md"), Path("ARCHITECTURE.md"), Path("context.md"),
    ):
...
def ensure_runs_link(source_root: Path, target_root: Path) -> None:
    ...
    if target_runs.is_symlink():
        ...
        shutil.rmtree(target_runs) if target_runs.is_dir() else target_runs.unlink()
...
def remove_runtime_links(repo_root: Path) -> None:
    for relative in (Path(".harness/runs"), Path(".harness/workflows"), Path(".codex/agents"), Path(".codex/skills"), Path("harness"), Path("harness_codex")):
...
def committable_status_paths(status_text: str) -> tuple[str, ...]:
    ...
    excluded = (".harness/runs", ".harness/workflows", ".codex/agents", ".codex/skills", "harness", "harness_codex", "venv")
    paths: list[str] = []
    ...
    for entry in entries:
        ...
        if path and not any(path == prefix or path.startswith(prefix + "/") for prefix in excluded):
            paths.append(path)
