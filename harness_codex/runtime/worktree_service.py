...
from harness_codex.runtime.worktree_support import (
    add_worktree, branch_contains, committable_status_paths, git,
    hydrate_runtime_worktree, is_git_worktree, remove_runtime_links,
    remove_worktree, safe_ref_part, usable_worktree, worktree_dirty,
    worktrees_base_dir,
)
...
class WorktreeService:
    ...
    def prepare(self, repo_root: Path, change_set_id: str, run_id: str) -> WorktreeIsolation | None:
        if not is_git_worktree(repo_root):
            return None
        safe_change, safe_run = safe_ref_part(change_set_id), safe_ref_part(run_id)
        ...
        return WorktreeIsolation(repo_root, integration_root, integration_branch, {}, {})
    ...
    def work_item_root(self, isolation: WorktreeIsolation | None, scope) -> Path | None:
        ...
        prefix = safe_ref_part(isolation.integration_branch.replace("/", "-"))
        branch = f"harness/{prefix}/{safe_item}"
    ...
    def commit_and_merge(self, isolation: WorktreeIsolation, scope, result: RunResult, *, change_set_id: str) -> RunResult:
        worktree = isolation.work_item_roots[scope.display_id]
        branch = isolation.work_item_branches[scope.display_id]
        baseline = self.commit_if_dirty(isolation.integration_root, f"{change_set_id} 전달 기준 산출물 반영")
        if baseline.returncode != 0:
            return self.blocked_result(result, "delivery baseline commit failed", baseline)
        committed = self.commit_if_dirty(worktree, f"{change_set_id} {scope.display_id} 구현 완료")
        if committed.returncode != 0:
            return self.blocked_result(result, "work-item commit failed", committed)
        merged = git(isolation.integration_root, "merge", "--no-ff", "--no-edit", branch, check=False)
        if merged.returncode != 0:
            git(isolation.integration_root, "merge", "--abort", check=False)
            return self.blocked_result(result, "work-item merge failed", merged)
        return replace(result, metadata={**dict(result.metadata), "worktree_root": str(worktree), "worktree_branch": branch, "integration_worktree": str(isolation.integration_root), "integration_branch": isolation.integration_branch})
    ...
    def blocked_result(self, result: RunResult, message: str, completed: subprocess.CompletedProcess[str]) -> RunResult:
        detail = completed.stderr.strip() or completed.stdout.strip() or message
        return replace(result, status=RunStatus.BLOCKED, failed_step_id=result.failed_step_id or "worktree-isolation", failure_kind=result.failure_kind or FailureKind.IMPLEMENTATION, blocker=f"{message}: {detail}", metadata={**dict(result.metadata), "worktree_isolation_error": detail})
...
def _sync_resumed_active_plan(source_root: Path, worktree_root: Path, scope) -> None:
    if (worktree_root / _completed_plan_path(scope.display_id)).exists():
        return
    source = source_root / _active_plan_path(scope)
    target = worktree_root / _active_plan_path(scope)
    if source.is_file():
