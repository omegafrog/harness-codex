...
def test_failed_specialist_result_blocks_dependents_even_when_xml_claims_completed(tmp_path: Path, monkeypatch) -> None:
    source = Path(__file__).parents[1]
    workflow = tmp_path / ".harness/workflows"
    workflow.mkdir(parents=True)
    (workflow / "changeset-use-case-workflow.yaml").write_text((source / ".harness/workflows/changeset-use-case-workflow.yaml").read_text(encoding="utf-8"), encoding="utf-8")
    monkeypatch.setattr("harness_codex.orchestration.runtime_dispatch.dispatch_specialist", lambda **_: SimpleNamespace(status="blocked", fact="subagent_protocol_failure"))
    step_dir = tmp_path / ".harness/runs/run-1/steps/plan-work-item"
    result = ET.Element(f"{{{RESULT_NS}}}subagent-result", {"schemaVersion": "1"})
    ...
    ET.SubElement(outcome, f"{{{RESULT_NS}}}summary").text = "invalid accepted result"
    ET.SubElement(result, f"{{{RESULT_NS}}}artifacts")
    ET.SubElement(result, f"{{{RESULT_NS}}}evidence")
    ET.SubElement(result, f"{{{RESULT_NS}}}changes")
    ET.SubElement(result, f"{{{RESULT_NS}}}blockers")
    ...
    assert dispatch(repo_root=tmp_path, run_id="run-1", step_id="plan-work-item", change_set_id="CHG-1", work_item_id="MAINT-1") == ("blocked", "subagent_protocol_failure")
    status, fact = dispatch(repo_root=tmp_path, run_id="run-1", step_id="review-work-item-plan", change_set_id="CHG-1", work_item_id="MAINT-1")

    assert (status, fact) == ("blocked", "unmet_needs:plan-work-item")
