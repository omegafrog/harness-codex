...
def test_changes_continue_routes_maintenance_through_plan_and_implementation(
    tmp_path: Path,
    capsys,
    monkeypatch,
) -> None:
    write_maintenance_changeset(tmp_path)
    change_set_path = tmp_path / "docs/changes/active/CHG-002.md"
    text = render_initial_changeset(
        change_set_id="CHG-002",
        title="Maintenance work",
        request_summary="Clean test gate",
    )
    text += """
...
"""
    ...
    assert "Target stage: plan-writing" in first_output
    assert "Work item: MAINT-001" in first_output

    active_plan = tmp_path / "docs/plans/active/MAINT-001/plan.md"
    ...
    assert "Target stage: implementation" in second_output
    assert captured == [
        {"stage": "plan-writing", "work_item": "MAINT-001", "uc": ""},
        {"stage": "implementation", "work_item": "MAINT-001", "uc": ""},
    ]


def test_changes_continue_does_not_recreate_completed_work_item_plan(
    tmp_path: Path,
    capsys,
) -> None:
    write_maintenance_changeset(tmp_path)
    completed = tmp_path / "docs/plans/completed/MAINT-001/plan.md"
    ...
    output = capsys.readouterr().out
    assert exit_code == 0
    assert output == "BLOCKED: MAINT-001 plan is already completed\n"
