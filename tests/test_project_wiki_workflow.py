...
def test_project_wiki_assets_remain_available_as_an_explicit_operation() -> None:
    ...
    skill = read(
        ".codex/skills/harness-project-wiki/SKILL.md"
    )
    details = read(
...
def test_work_item_execution_keeps_wiki_explicit_and_delivery_in_finalization() -> None:
    ...
    assert "update-project-wiki" not in work_item_step_ids
    assert "validate-project-wiki" not in work_item_step_ids
    ...
    assert finalization_workflow.step_by_id("create-change-set-pr").metadata["execution_boundary"] == "changeset_finalization"
