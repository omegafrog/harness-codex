...
_harness_complete_memory() {
  local current_word="${COMP_WORDS[COMP_CWORD]}"
  COMPREPLY=( $(compgen -W "list search reindex" -- "$current_word") )
}
...
_harness_completion() {
  ...
  local current_word="${COMP_WORDS[COMP_CWORD]}"
  local procedure_options="--uc --title --idea --force --force-verification --rollback --plan --preview --apply"
  local commands="help init update reset agent-context changes contracts completion run requirements-definition ubiquitous-language-definition use-case-definition event-storming ddd-architecture-definition technical-decisions plan-writing implementation memory evolution stages artifacts resume report dashboard ui-server"
  ...
  if [[ "$command" == "changes" && "$subcommand" == "continue" && $COMP_CWORD -eq 4 ]]; then
    COMPREPLY=( $(compgen -W "--uc --blocker-resolution --resolution-prompt --force-verification --plan --preview --apply" -- "$current_word") )
    return 0
  fi
  ...
  if [[ "$command" == "run" && "$subcommand" == "app" && $COMP_CWORD -eq 3 ]]; then
    COMPREPLY=( $(compgen -W "init dev prod start stop health deploy delete logs usage env status attach --foreground --timeout --env --apply --volumes --follow --tail --json --force" -- "$current_word") )
    return 0
  fi
  ...
  if [[ "$command" == "memory" && $COMP_CWORD -eq 2 ]]; then
    _harness_complete_memory
    return 0
  fi
