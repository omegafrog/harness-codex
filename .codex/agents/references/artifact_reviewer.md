...
- Return a concise approval/rejection with findings and evidence.
...
- New plans declare exactly one Verification Profile level. Verify each requirement has an accurate layer and
  the probe set can prove the declared level. Health, Swagger, or UI-root checks are smoke only; fake-provider
  or direct-fixture tests are component evidence and cannot prove `live_e2e_ready` without an approved waiver.
