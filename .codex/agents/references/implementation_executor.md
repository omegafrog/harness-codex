...
- When the repository contains a runnable application, create or update `scripts/run-app.sh` as part of implementation. Keep required local infrastructure in versioned code such as `compose.yaml`, Dockerfiles, migrations, and bootstrap scripts. The launcher must run from the repository root and start the complete local application path.
