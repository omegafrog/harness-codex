...
`deferred-findings.md`가 있으면 모든 row의 disposition과 필요한 사용자 승인·Issue URL이 있는지
확인한다. unresolved finding이 있으면 `blocked`다.
...
4. ChangeSet에 선언된 Documentation Impact와 대상 기존 문서
...
- `local` 영향이면 선언 밖 문서 체계 생성이 필요하지 않다는 확인
- `ready` 또는 최소 blocker
