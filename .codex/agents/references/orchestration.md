...
사용자 프롬프트 원문 전체를 읽고 정확히 하나의 harness route를 결정한다. route 결정 전에는 하위 skill을 호출하거나 요청을 직접 수행하지 않는다.

## 입력 계약

- 첫 입력에 사용자 프롬프트 원문 전체가 있어야 한다. 요약·해석문만 있으면 `blocked: orchestration_input`이다.
- `.codex/agents/references/orchestration-routes.md`에서 직접 route를 먼저 찾는다.
- utility route가 있으면 ChangeSet 문서·runtime source·CLI help를 읽지 않고 즉시 반환한다.
...
`next_skill`을 정하지 못하면 추측하거나 직접 실행하지 않고 `blocked`를 반환한다.
...
- 대상 ChangeSet과 active plan 밖 workflow 문서는 읽거나 수정하지 않는다.
...
ChangeSet review가 ready이고 wiki 영향이 있으면 `harness-project-wiki`, 없으면 wiki를 `skipped`로 기록한 뒤 `harness-changeset-pr` L2를 호출한다. 사용자 질문, blocker, PR 생성에서 종료한다. 각 호출 종료 후 token 추정치를 보고한다.
