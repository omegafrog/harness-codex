...
검증 command의 observation budget은 workflow metadata `verification_observation_budget_sec`를 사용하고, 없으면 90초다. budget을 넘긴 command는 성공/실패 timeout이 아니라 관측 전환 대상이다. command process만 종료한 뒤 같은 명령을 재시도하지 않고 최소 증거로 원인을 분류한다. 증거는 현재 명령의 로그, stack trace, 실패한 테스트, 또는 관련된 최소 코드·외부 상태만 사용한다.
