...
- Apply the observed-problem-resolution protocol. At workflow `verification_observation_budget_sec` (default `90`), stop only the over-budget command and collect current command output, stack when available, and the smallest related test/source evidence. When a missing root-cause task is outside the active plan boundary, return that evidence with `failure code="verification_root_cause"` for planner remediation and stop; do not use delay, retry, or environment workaround as a substitute for a fix.
