...
- Apply the observed-problem-resolution protocol. Read `verification_observation_budget_sec` from `execution-scope.xml` before the first verification command. Start every potentially long verification command under `timeout --signal=TERM --kill-after=10s <budget>s sh -c '<command>'`; do not run it raw and try to stop it later. At budget expiry, retain command output, collect stack when available and the smallest related test/source evidence. When a missing root-cause task is outside the active plan boundary, return that evidence with `failure code="verification_root_cause"` for planner remediation and stop; do not use delay, retry, or environment workaround as a substitute for a fix.
