...
- Require the plan to state execution scope, `implementationBoundary`, package/dependency contract, domain implementation contract, external-contract read allowlist, task checklist, and focused verification. Report a blocker when a required decision is missing, contradictory, or a placeholder.
...
- For non-evolve runs, do not write runtime, agent, skill, workflow, control-plane, generated runtime output, or read-only context files. Harness-owned protected targets include `AGENTS.md`, `**/AGENTS.md`, `.codex/**`, `.semgrep/**`, `.harness/system/**`, `.harness/agents/**`, `.harness/contracts/**`, `.harness/docs/**`, `.harness/workflows/**`, `.harness-codex/**`, `harness_codex/**`, `tests/runtime/**`, `completions/**`, the root `harness` launcher, `scripts/install-harness-codex.sh`, and `scripts/bump_runtime_version.py`.
- Modify application source only inside the active plan's `implementationBoundary.source` patterns.
- Modify tests only inside the active plan's `implementationBoundary.tests` patterns.
- Modify build/config/script files only when the exact path or narrow pattern is listed in `implementationBoundary.configExceptions`. Do not treat build files, scripts, Docker/Compose files, env templates, or resources XML/YAML/properties files as blanket writable support files.
- Write runtime artifacts only under the runtime-owned paths declared in `implementationBoundary.runtimeArtifacts` or supplied by the runtime.
- If a required edit is outside `implementationBoundary.source`, `implementationBoundary.tests`, or `implementationBoundary.configExceptions`, do not edit it. Stop and return `scopeExpansionRequest: <path-or-module> | <reason> | <why current boundary is insufficient> | <expected test impact>`.
