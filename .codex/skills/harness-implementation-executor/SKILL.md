...
Perform one runtime-dispatched implementation attempt for the active work item. Complete the plan's unchecked implementation tasks and return a bounded XML execution report.
...
- When the plan includes app runtime work, implement the full lifecycle exactly as planned: development Docker image build/start/stop/health scripts, production scripts only from the approved production operations Markdown, env templates without real secrets, and stop scripts that verify owned containers/infrastructure are down.
- Record focused verification commands and results in runtime evidence files and the runtime-declared `execution-report.xml`.
...
- Copy `plan_fingerprint` from `execution-scope.xml` into `execution-report.xml` so the report is bound to the exact plan that was executed.
...
- Report changed files, completed tasks, remaining tasks, focused verification results, evidence paths, and blockers in `execution-report.xml` using the fixed `execution-report` XML handoff envelope.
