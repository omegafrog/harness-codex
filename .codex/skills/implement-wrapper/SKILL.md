...
The `implement-wrapper` parent and every normal `implement` execution subagent must use `model: agents.implementation_model` with `reasoning_effort: high`. The skill cannot change an already-running parent model; select the configured implementation pair before invocation when the runtime exposes model selection.
...
- Resolve `agents.implementation_model` and `agents.implementation_reasoning_effort` from `.codex/harness.yaml`; implementation reasoning must be `high`. Fall back to `agents.default_model` only for older configurations.
...
- Start every plan ticket with a newly spawned `implement` subagent whose execution context contains no prior plan work, `model: agents.implementation_model`, and `reasoning_effort: high`. Never reuse a completed, handed-off, or blocked plan's subagent for the next plan.
...
- If blocker resolution requires a new architecture decision, cross-plan reconciliation, or complex root-cause analysis, explicitly escalate the fresh recovery subagent to `model: agents.high_performance_model` with `reasoning_effort: agents.high_performance_reasoning_effort`; ordinary retries remain on `agents.implementation_model` with `reasoning_effort: high`.
