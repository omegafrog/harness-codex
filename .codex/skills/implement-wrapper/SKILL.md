...
The `implement-wrapper` parent and every normal `implement` execution subagent must use `model: agents.implementation_model` with `reasoning_effort: agents.implementation_reasoning_effort`. The skill cannot change an already-running parent model; select the configured implementation pair before invocation when the runtime exposes model selection.
...
- Resolve `agents.implementation_model` and `agents.implementation_reasoning_effort` from `.codex/harness.yaml`. Fall back to `agents.default_model` only for older configurations.
...
- Start every plan ticket with a newly spawned `implement` subagent whose execution context contains no prior plan work, `model: agents.implementation_model`, and `reasoning_effort: agents.implementation_reasoning_effort`. Never reuse a completed, handed-off, or blocked plan's subagent for the next plan.
...
At every plan boundary, checkpoint the completed plan and dispatch the next eligible plan only to a new empty-context subagent with `model: agents.implementation_model` and `reasoning_effort: agents.implementation_reasoning_effort`, even when the previous agent has remaining context. Record `handoff_reason: plan-boundary`; a plan boundary is never a reason to continue with the previous agent.
...
- If blocker resolution requires a new architecture decision, cross-plan reconciliation, or complex root-cause analysis, explicitly escalate the fresh recovery subagent to `model: agents.high_performance_model` with `reasoning_effort: agents.high_performance_reasoning_effort`; ordinary retries remain on `agents.implementation_model` with `reasoning_effort: agents.implementation_reasoning_effort`.
- If the plan requires actual E2E or server execution, dispatch a fresh `execution_runner` subagent with `model: agents.execution_model` and `reasoning_effort: agents.execution_reasoning_effort`. It owns process startup, log collection, polling until completion or timeout, and result reporting; it does not edit implementation files.
