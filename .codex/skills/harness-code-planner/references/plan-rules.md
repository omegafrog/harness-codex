...
- Do not include runtime control files, agent context, read-only context, review artifacts, verification-tool configuration, generated reports, `.harness/docs/**` harness documentation/templates, or unrelated module outputs in the executor write boundary. Valid implementation write paths are product source files, tests, directly required build files, framework/runtime configuration, cache configuration, migration/init SQL, Docker/Compose files, env templates, and maintained execution scripts. If verification mentions a tool config such as `.semgrep/**`, treat it as read-only verification input unless the ChangeSet explicitly requests tooling changes.
- Before finalizing `## 실행 경계`, build an implementation-support file inventory from repository layout, build files, runtime configuration, technical decisions, and focused verification needs. Include exact paths for known config/build/script/cache files such as `src/main/resources/application*.yml`, `src/main/resources/ehcache.xml`, migration/init SQL, `build.gradle*`, `settings.gradle*`, `pom.xml`, `compose*.yml`, `Dockerfile*`, `scripts/**`, and `config/runtime/**` when the active work clearly requires writes there. Missing a project-owned support file is not itself a plan-rejection reason; the source-code module boundary remains the hard scope constraint.
- Do not write blanket forbidden-path bullets that exclude whole support-file classes such as cache XML, application config, build manifests, Docker/Compose, env templates, or maintained scripts when those files may be needed for focused verification or runtime readiness. `### 수정 금지 경로` should stay specific to truly out-of-scope source modules, bounded contexts, generated outputs, or read-only control-plane paths.
- If a support file is outside the main source module but may be required to satisfy the work-item goal, prefer either listing the exact file in `### 수정 허용 경로` or leaving it discoverable under the repository support-file policy. Do not create plan text that forces the reviewer to conclude "required support file but forbidden by plan" unless that contradiction is genuinely intended and should block planning.
...
- Executor must not edit active-plan checkbox markers. Execution results belong in its concise specialist result.
- Executor uses the plan checklist as instructions and reports completed/remaining tasks to the current-session orchestrator.
...
- Do not put prior execution evidence paths in the active plan. Keep execution evidence in the specialist result.
...
- Keep the plan at `docs/plans/active/<WORK-ITEM-ID>/plan.md` until focused verification and required reviews pass.
- Keep the plan active if execution evidence is missing, stale, incomplete, or failed.
...
- The current-session orchestrator may complete the active-to-completed transition only when:
  ...
  - verification results and evidence paths are reported by the executor
