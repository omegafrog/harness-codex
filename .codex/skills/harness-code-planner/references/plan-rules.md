...
- Every runtime environment script set must provide start, stop, and health behavior. Stop scripts must verify that all containers and infrastructure owned by that environment are down; production stop scripts must also verify cloud containers/services/tasks/pods/jobs are stopped or removed using the commands approved in the production operations Markdown.
