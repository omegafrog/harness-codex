...
- When a deterministic gate reports only a legacy ChangeSet Impact Type value (`new`, `changed`, `create`, or another action-style label), route one bounded `legacy-impact-tag-migration` to `change_set_bootstrapper`. It updates only the active ChangeSet table cell(s) to canonical impact tags, then re-run the failed gate. Do not accept legacy tags in the verifier and do not block before this producer repair is attempted.
