...
각 workflow 실행은 `origin/main` 또는 `HEAD` 기반 sibling worktree 하나와 선택된 ChangeSet·work item slice·active plan만 사용한다.
...
docs/plans/active/<WORK-ITEM-ID>/
  plan.md
  verification.md
```

- 선택된 `<CHG-ID>`, work item slice, active plan 밖 workflow 문서는 읽거나 수정하지 않는다.
...
- worktree 생성 step은 `.codex/workflow/changeset-template.md`로 `docs/changes/active/<CHG-ID>/changeset.md` skeleton을 만든다. orchestrator는 초기 요청·범위·intent·대상 work item만 채운다.
