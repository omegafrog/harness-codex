...
- plan-set implementation PR 하나만 만들고, 본문에는 parent와 모든 child에 대한 closing keyword를 각각 넣는다. 구현 완료 직후에는 Issue를 닫지 않으며, PR이 repository default branch에 merge된 뒤 GitHub가 Issue를 닫고 Project `Workflow Status`를 `Done`으로 반영한 뒤 dependent를 재평가한다.
