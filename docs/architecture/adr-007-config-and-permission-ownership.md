...
`.codex/harness.yaml`은 eval defaults, suite/case paths, journal/worktree policy, gate thresholds와 native permission profile reference만 소유한다. Secret, credential, raw permission rule은 저장하지 않는다. Agent profile은 role definition, model defaults, context policy와 permission profile reference를 소유한다.
